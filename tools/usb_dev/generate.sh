...
# Configuration
PCI_IDS_URL="https://usb-ids.gowdy.us/usb.ids"
PCI_IDS_FILE="usb.ids"
PY_SCRIPT="usb_dev.py"
OUTPUT_H="../../src/kernel/klibc/include/drivers/usb/devices/usb_dev.h"
FILTER_FILE="filter.txt"

# Download/Update usb.ids if it doesn't exist
if [ ! -f "$PCI_IDS_FILE" ]; then
    echo "Downloading usb.ids..."
    wget -q --show-progress $PCI_IDS_URL -O $PCI_IDS_FILE
else
    echo "usb.ids already exists, skipping download"
fi
