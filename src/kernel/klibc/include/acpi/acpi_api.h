...
extern "C" {
	...
	} wallos_acpi_dev_t; // The name is to try to avoid collisions, I have a feeling that acpi_device is probably defined elsewhere
	...
	/**
	 * @brief Helper function to determine if the ACPI subsystem has been fully initialized yet.
	 *
	 * This is updated (only one direction, false -> true) by calling acpi_set_setup_completed().
	 *
	 * This is meant to be used in places that may need to handle things different before/after startup (like PCI).
	 *
	 * @return true If subsystem is done initializing.
	 * @return false If subsystem is not done initializing or failed to initialize.
	 */
	bool acpi_setup_complete(void);

	/**
	 * @brief Called at the end of ACPI startup by the ACPI subsystem.
	 *
	 * Once called, acpi_setup_complete() will always return true.
	 */
	void acpi_set_setup_completed(void);

	/**
	 * @brief Power off the system (S5)
	 *
	 * If the system doesn't have S5 (or fails for some reason), this will halt *all* processor operations on all cores, then alert the user.
	 */
	__attribute__((noreturn)) void acpi_shutdown(void);

	/**
	 * @brief Reboot the system via ACPI reset register (if present), or legacy ways otherwise.
	 */
	__attribute__((noreturn)) void acpi_reboot(void);

	/**
	 * @brief Enter a sleep state (S1-S4)
	 *
	 * Currently unsupported, this needs a lot of work.
	 *
	 * @param state State to enter. The only acceptable range is 1, 2, 3, & 4 inclusive.
	 * @return acpi_status_t Errors if they occur.
	 */
